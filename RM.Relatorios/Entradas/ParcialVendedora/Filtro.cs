...
namespace RM.Relatorios.Entradas.ParcialVendedora
{
    public partial class Filtro : Form
    {
        ...
        private void CarregaVendedoras()
        {
            ...
            List<Dados.TVEN> gerentes = Lib.Vendedora.GetByFilial(Filial.CODCOLIGADA, Filial.CODFILIAL).Where(a => a.VENDECOMPRA == (short)Lib.Enums.Funcoes.Vendedor).ToList();

            //carrega combo
            comboVendedora.ValueMember = "CODVEN";
            comboVendedora.DisplayMember = "NOME";
            comboVendedora.DataSource = gerentes;
        }

        private void CarregaRelatorio()
        {
            Resultado frm = new Resultado(GetResult(), comboFilial.Text, comboVendedora.Text);
            frm.Show();
        }
        ...
        private List<Model> GetResult()
        {
            ...
            List<Dados.FLAN> lancamentos = Lib.Lancamento.GetParcialByVendedora(filial, vendedora);
            ...
            foreach (Dados.FLAN item in lancamentos)
            {
                ...
                modelo.Percentual = vendedora.COMISSAO1 == null ? 2 : (decimal)vendedora.COMISSAO1;
