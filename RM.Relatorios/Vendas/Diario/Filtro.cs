...
namespace RM.Relatorios.Vendas.Diario
{
    public partial class Filtro : Form
    {
        ...
        private List<Model> GetResult()
        {
            ...
            List<Dados.TMOV> vendas = Lib.Movimento.GetVendasByEmissao(filial, dataInicio.Value, dataFim.Value);
            ...
            foreach (Dados.TMOV item in vendas)
            {
                ...
                if (item.FCFO.FCFOCOMPL.CODCMASTER != null)
                    ...
                    modelo.CodCmaster = 0;
                modelo.DataEmissao = item.DATAEMISSAO.Value;
                if (item.TMOVCOMPL.DTLIBERACAO != null)
                    modelo.DataLiberacao = item.TMOVCOMPL.DTLIBERACAO.Value;
                else
                    modelo.DataLiberacao = item.DATAEMISSAO.Value;
                ...
                modelo.TipoVenda = Lib.Movimento.GetTipoVenda(item.CODTB1FLX);
